...
from cdglib.models_domain.meter_configs.meter_config_delta import MeterConfigDeltaBase
from .meter_config_base import MeterConfig


class MeterConfigDelta(MeterConfig, MeterConfigDeltaBase):
    __tablename__ = 'meter_config_delta'

    id = Column(Integer, ForeignKey('clusterschema.meter_config.id'), primary_key=True)
    __mapper_args__ = {'polymorphic_identity': 'MeterConfigDelta', 'inherit_condition': id == MeterConfig.id}
