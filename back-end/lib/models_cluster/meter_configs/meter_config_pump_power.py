...
from cdglib.models_domain.meter_configs.meter_config_pump_power import MeterConfigPumpPowerBase
from .meter_config_base import MeterConfig


class MeterConfigPumpPower(MeterConfig, MeterConfigPumpPowerBase):
    __tablename__ = 'meter_config_pump_power'

    id = Column(Integer, ForeignKey('clusterschema.meter_config.id'), primary_key=True)
    __mapper_args__ = {'polymorphic_identity': 'MeterConfigPumpPower', 'inherit_condition': id == MeterConfig.id}
